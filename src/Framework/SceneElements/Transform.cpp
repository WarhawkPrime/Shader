...
//unabhängig von rotation und skalierung wird objekt translatiert
void Transform::translate(const glm::vec3 & deltaPos)
{
    m_position += deltaPos;
    m_matrixDirty = true;
}
//abhängig von rotation und skalierung wird objekt translatiert
...
glm::vec3 Transform::getCombinedPosition() {
    glm::mat4 matrix = this->getCombinedMatrix();
    glm::vec3 position = glm::vec3(matrix[3][0], matrix[3][1], matrix[3][2]);
    return position;
}
glm::mat4 Transform::getCombinedMatrix() {
    if (this->getParent() != nullptr) {
        return this->getParent()->getCombinedMatrix() * this->getTransformMatrix();
    }
